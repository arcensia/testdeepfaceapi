...
@blueprint.route("/findName", methods=["POST"])
def findName():
    input_args = request.get_json()
    if input_args is None:
        return {"message": "empty input set passed"}

    img_path = input_args.get("img1") or input_args.get("img1_path")
    db_path = input_args.get("img2") or input_args.get("img2_path")
    ...
    demographies = service.find_name(
        img_path=img_path,
        db_path=db_path,
        model_name=model_name,
        detector_backend=detector_backend,
